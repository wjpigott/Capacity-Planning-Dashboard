...
function Ensure-AzureContext {
    ...
    function Connect-SelfHostedAzureAccount {
        $authMode = [string]$env:CAPACITY_AZURE_AUTH_MODE
        if ([string]::IsNullOrWhiteSpace($authMode)) {
            return $false
        }

        $tenantId = [string]$env:CAPACITY_AZURE_TENANT_ID
        $clientId = [string]$env:CAPACITY_AZURE_CLIENT_ID
        if ([string]::IsNullOrWhiteSpace($tenantId) -or [string]::IsNullOrWhiteSpace($clientId)) {
            throw 'CAPACITY_AZURE_TENANT_ID and CAPACITY_AZURE_CLIENT_ID are required for self-hosted service principal authentication.'
        }

        switch ($authMode.Trim().ToLowerInvariant()) {
            'service-principal-secret' {
                $clientSecret = [string]$env:CAPACITY_AZURE_CLIENT_SECRET
                if ([string]::IsNullOrWhiteSpace($clientSecret)) {
                    throw 'CAPACITY_AZURE_CLIENT_SECRET is required when CAPACITY_AZURE_AUTH_MODE=service-principal-secret.'
                }

                $secureSecret = ConvertTo-SecureString $clientSecret -AsPlainText -Force
                $credential = [System.Management.Automation.PSCredential]::new($clientId, $secureSecret)
                $null = Connect-AzAccount -ServicePrincipal -Tenant $tenantId -Credential $credential -ErrorAction Stop
                return $true
            }
            'service-principal-certificate' {
                $certificatePath = [string]$env:CAPACITY_AZURE_CLIENT_CERTIFICATE_PATH
                if ([string]::IsNullOrWhiteSpace($certificatePath) -or -not (Test-Path -LiteralPath $certificatePath -PathType Leaf)) {
                    throw 'CAPACITY_AZURE_CLIENT_CERTIFICATE_PATH must reference an existing certificate file when CAPACITY_AZURE_AUTH_MODE=service-principal-certificate.'
                }

                $certificatePassword = [string]$env:CAPACITY_AZURE_CLIENT_CERTIFICATE_PASSWORD
                $parameters = @{
                ...
                if (-not [string]::IsNullOrWhiteSpace($certificatePassword)) {
                    $parameters.CertificatePassword = ConvertTo-SecureString $certificatePassword -AsPlainText -Force
                }
                $null = Connect-AzAccount @parameters
                return $true
            }
            default {
                throw "Unsupported CAPACITY_AZURE_AUTH_MODE '$authMode'. Use service-principal-secret or service-principal-certificate."
            }
        }
    }
    ...
    try {
        if (Connect-SelfHostedAzureAccount) {
        ...
        $Caches.LoginAttempted = $true
        $Caches.LastPlacementWarning = "Self-hosted service principal sign-in failed: $($_.Exception.Message)"
        return $false
    }
...
function Get-PlacementScores {
    ...
    if (-not (Get-Command -Name 'Invoke-AzSpotPlacementScore' -ErrorAction SilentlyContinue)) {
        $restDetail = if ($Caches.LastPlacementRestError) { " ARM REST fallback failed: $($Caches.LastPlacementRestError)" } else { '' }
        $Caches.LastPlacementWarning = "Live placement lookup is unavailable because Invoke-AzSpotPlacementScore is not installed.$restDetail"
        return $scores
    }
