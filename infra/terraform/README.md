...
| Virtual Network + 2 subnets | `azurerm_virtual_network.vnet`, `azurerm_subnet` (×2), or existing VNet/subnet data sources when existing network names are supplied |
...
- Shared secrets for the dashboard runtime
    - optional `ingest_api_key` override; generated by Terraform when empty
    - optional `session_secret` override; generated by Terraform when empty
...
# Optional: omit these to let Terraform generate and persist random values in state.
...
| `ingest_api_key` | `""` | Shared secret for ingestion routes; generated by Terraform when empty (sensitive) |
| `session_secret` | `""` | Session middleware secret; generated by Terraform when empty (sensitive) |
...
| `existing_virtual_network_name` | `""` | Existing virtual network name to reuse; skips VNet/subnet creation when supplied with both subnet names |
| `existing_virtual_network_resource_group_name` | `""` | Optional resource group override for the existing virtual network |
...
- For customer or separate-tenant network handoff testing, set `existing_virtual_network_name`, `existing_app_service_integration_subnet_name`, and `existing_private_endpoint_subnet_name` together. `existing_virtual_network_resource_group_name` is optional and defaults to `resource_group_name`.
- In existing-network mode Terraform does not create or modify the VNet or subnets. The App Service integration subnet must already be delegated to `Microsoft.Web/serverFarms`, and the private endpoint subnet must already allow SQL/Key Vault private endpoints when Terraform creates those private endpoints.
