...
const {
  getLitePaaSDatabaseQuotaSnapshot,
  refreshLitePaaSDatabaseQuotaSnapshot
} = require('./services/litePaaSDatabaseQuotaService');
...
function requireLiteAdmin(req, res, next) {
...
app.get('/api/auth/me', (req, res) => {
  ...
  const localUsername = process.env.USERNAME || process.env.USER || 'local user';
  ...
  res.json({
    ...
    auth: {
      authEnabled,
      isAuthenticated,
      name: account?.name || (!authEnabled ? `Local session: ${localUsername}` : null),
      username: account?.username || (!authEnabled ? 'Authentication disabled' : null),
...
app.get('/api/paas-db-quota', async (req, res) => {
  try {
    const result = IS_LITE_DEPLOYMENT
      ? await getLitePaaSDatabaseQuotaSnapshot()
      : await getPaaSDatabaseQuotaSnapshot({ maxAgeHours: req.query.maxAgeHours });
    res.json(result);
  } catch (err) {
...
app.post('/api/paas-db-quota/refresh', requireLiteAdmin, async (req, res) => {
  try {
    const result = IS_LITE_DEPLOYMENT
      ? await refreshLitePaaSDatabaseQuotaSnapshot({
          services: req.body?.services || req.body?.service || ['All'],
          includeCapabilities: req.body?.includeCapabilities !== false
        })
      : await runPaaSDatabaseQuotaScan({
          subscriptionIds: req.body?.subscriptionIds,
