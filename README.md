...
For an existing Lite deployment, Capacity Admins can update the snapshot scope after deployment in **Admin -> Data Ingestion**. Enter one or more comma-separated subscription IDs and/or management-group names, set the Azure **Capture Regions** directly or load a region preset, select **Save Azure Scope**, then select **Refresh Report Data**. A non-empty saved capture list overrides the worker's `CAPACITY_REPORT_REGIONS`; leaving it blank retains that deployment fallback. The saved scope is stored at `capacity-report-snapshots/scope.json`; the next snapshot populates the report and sidebar subscription filter from its captured subscriptions.
